...
data(wheat)
Y<- wheat.Y   ## Y <- read.table("phenotype.txt", head = TRUE)
X<- wheat.X   ## X <- read.table("genotype.txt", head = TRUE)
A<- wheat.A   ## A <- read.table("Kinship.txt", head = TRUE)
...
model_cnn <- train(train_data, train_phenotype,
                   trControl = control,
                   tuneGrid = expand.grid(layer1_filter = 16, layer1_kernel = c(2, 3),
                                          layer2_filter = 32, layer2_kernel = c(2, 3)))
